...
class BoseSoundTouch extends EventEmitter {
	 ...
	 */
		...
		<updates deviceID="0A1B2C3D4E5F"><zoneUpdated><zone master="04A316E14903"><member ipaddress="192.168.2.188">0A1B2C3D4E5F</member></zone></zoneUpdated></updates>
		{ zone: [ { '$': [Object], member: [Array] } ] }
		Bose-Cuisine (0A1B2C3D4E5F) received zoneUpdated notification
		event not yet treated
		<updates deviceID="0A1B2C3D4E5F"><zoneUpdated><zone master="04A316E14903" senderIPAddress="192.168.2.246" senderIsMaster="true"><member ipaddress="192.168.2.188">0A1B2C3D4E5F</member></zone></zoneUpdated></updates>
		{ zone: [ { '$': [Object], member: [Array] } ] }

		REM:

		<updates deviceID="0A1B2C3D4E5F"><zoneUpdated><zone /></zoneUpdated></updates>
		{ zone: [ '' ] }
		Bose-Cuisine (0A1B2C3D4E5F) received nowPlayingUpdated notification
		Bose-Cuisine (0A1B2C3D4E5F) source: STANDBY, playing: ERROR
		{}
		Bose-Cuisine (0A1B2C3D4E5F) received zoneUpdated notification
		event not yet treated
		<updates deviceID="0A1B2C3D4E5F"><zoneUpdated><zone /></zoneUpdated></updates>
...
/*
		...
		{ '$': { master: '04A316E14903' },
		  member: [ 
		  	{ _: '0A1B2C3D4E5F', '$': [Object] },
		  	{ _: '0A1B2C3D4E5F', '$': [Object] } 
		  ] }
		Bose-Cuisine (0A1B2C3D4E5F) received zoneUpdated notification
		{ '$': { master: '0A1B2C3D4E5F', senderIPAddress: '192.168.2.246', senderIsMaster: 'true' },
		  member: [ { _: '0A1B2C3D4E5F', '$': [Object] } ] }
*/
	}
	...
	parseNowPlaying( message) {
		...
		if ( current.powerOn !== powerOn) {
		 ...
		 /*
		<?xml version="1.0" encoding="UTF-8" ?>
		<nowPlaying deviceID="0A1B2C3D4E5F" source="SPOTIFY" sourceAccount="spotify-account">
		...
		<nowPlaying deviceID="0A1B2C3D4E5F" source="INTERNET_RADIO">
		...
		<nowPlaying deviceID="0A1B2C3D4E5F" source="BLUETOOTH" sourceAccount="">
	   ...
	   if (message.type === 'utf8') {
		...
		xmlParser.parseString( message.utf8Data, function( err, data) {
			...
			if ('SoundTouchSdkInfo' in data) {
				...
				/*
				 <errorUpdate deviceID="0A1B2C3D4E5F"><error value="1315" name="MUSIC_SERVICE_UNPLAYABLE_TRACK" severity="Unknown">kSpErrorContextFailed - Unable to read all tracks from the playing context. Playback of the Spotify context (playlist, album, artist, radio, etc) will stop early because eSDK is unable to retrieve more tracks. This could be caused by temporary communication or server problems, or by the underlying context being removed or shortened during playback (for instance, the user deleted all tracks in the playlist while listening.)</error></errorUpdate>
				 */
			}
				...
				for ( var key in data.updates) {
					...
					if( key === 'nowPlayingUpdated') {
						...
						// <updates deviceID="0A1B2C3D4E5F"><volumeUpdated><volume><targetvolume>44</targetvolume><actualvolume>44</actualvolume><muteenabled>false</muteenabled></volume></volumeUpdated></updates>
						...
						//console.log( message.utf8Data);
						// ignored
						// <updates deviceID="0A1B2C3D4E5F"><nowSelectionUpdated><preset id="1"><ContentItem source="INTERNET_RADIO" location="4712" sourceAccount="" isPresetable="true"><itemName>Studio Brussel</itemName><containerArt /></ContentItem></preset></nowSelectionUpdated></updates>
					}
					else if( key === 'sourcesUpdated') {
						// ignored
						// <updates deviceID="0A1B2C3D4E5F"><sourcesUpdated /></updates>
					}
					else if( key === 'recentsUpdated') {
						// console.log( message.utf8Data);
						// ignored
						// updates deviceID="0A1B2C3D4E5F"><recentsUpdated><recents><recent deviceID="0A1B2C3D4E5F" utcTime="1522107433" id="2221758507"><contentItem source="SPOTIFY" type="uri" location="spotify:station:user:spotify-account:cluster:3Cn3adRB3NJUpIpkucwi7G" sourceAccount="spotify-account" isPresetable="true"><itemName>Daily Mix 5</itemName></contentItem></recent><recent deviceID="0A1B2C3D4E5F" utcTime="1522107340" id="2174867728"><contentItem source="INTERNET_RADIO" location="4712" sourceAccount="" isPresetable="true"><itemName>Studio Brussel</itemName></contentItem></recent><recent deviceID="0A1B2C3D4E5F" utcTime="1521893943" id="2221589651"><contentItem source="SPOTIFY" type="uri" location="spotify:user:spotify:playlist:37i9dQZF1DWZn9s1LNKPiM" sourceAccount="7olx20j62dl29n5u11j3dzpgq" isPresetable="true"><itemName>90s Rock Renaissance</itemName></contentItem></recent><recent deviceID="0A1B2C3D4E5F" utcTime="1521887700" id="2221578653"><contentItem source="SPOTIFY" type="uri" location="spotify:user:spotify:playlist:37i9dQZF1DXb9LIXaj5WhL" sourceAccount="7olx20j62dl29n5u11j3dzpgq" isPresetable="true"><itemName>Bring Back the 90s</itemName></contentItem></recent><recent deviceID="0A1B2C3D4E5F" utcTime="1521885332" id="2215236660"><contentItem source="SPOTIFY" type="uri" location="spotify:station:user:spotify-account:cluster:7w4s5bpD2IzVoe4tE9lFh3" sourceAccount="spotify-account" isPresetable="true"> ....
					}
	...
	autosync() {
		...
		console.log(current + " do sync now & scheddule a sync in "+sec+" sec");
		this.sync();
	...
	_checkUpdateSuccess( err, res, body, handler) {
		...
		//console.log( current+" notification answer: "+body)
		//
	...
	play_url( url, title, handler) {
	   ...
	   if ((title == null) || (title == "")) {
           ...
           console.log( this+" play via DLNA title: "+title+" url: "+url);
	...
	*/
	  ...
	  var options = { 
		  ...
		  'url' : 'http://' + this.ip + ':' + dlna_port + "/AVTransport/Control",
		  'headers' : {

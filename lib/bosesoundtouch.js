...
class BoseSoundTouch extends EventEmitter {
	...
	parseConnectionState( message) {
		...
		console.log( current + " signal status: " + current.wifiSignal);
...
*/
	...
	_checkUpdateSuccess( err, data, res, handler) {
		...
		console.log( current+" response content-type: "+res.headers['content-type'])

		console.log( current+" notification answer: "+data.toString('utf8') )

		//check that buffer starts with "<xml"
		if (data.toString('utf8', 0, 5) != "<?xml" ) {
			console.log( current+" unrecognized content: "+res.headers['content-type'])
			console.log( current+" "+data.toString('utf8') )
	...
	*/
	  ...
	  urllib.request( 'http://' + this.ip + ':' + dlna_port + "/AVTransport/Control", options, (err, data, res) => { this._checkUpdateSuccess( err, data, res, handler) } );
	...
	*/
	...
	networkStatus( ) {
	  ...
	  this._telnet( 'network status', function( err, xml) {
		...
		console.log(current+" Fetching network status via telnet");

		if ( err) {
			console.log( current+" _telnet error: "+ err);
			return;
		}
		xmlParser.parseString( xml, function( err, data) {
			...
			if( ('Status' in data) && ('stationStats' in data.Status) && (Array.isArray( data.Status.stationStats))) {
				...
				console.log( current+" error while parsing status");
				console.log( data);
			}
		});
          });
